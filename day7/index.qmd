...
- Do NOT write `install.packages()` in your code
  - When I run it, it will install software on **my** computer. No thank you!

## Review

- Use `read_csv()`, not `read.csv()`.
  - The way they load data is slightly different. `read.csv()` is not *worse* necessarily, but it may result in unexpected things happening since I expect you to use `read_csv()`.

## Review

- Always put `library(tidyverse)` (or other packages) at the very top of your code, *before* `read_csv()` etc.
  - You need to load the package before you can use its functions.
...
- Please read the PDF with instructions posted to Moodle

- Please submit your choice of dataset if you haven't already