---
format:
  revealjs:
    incremental: false
    css: ../styles.css
execute: 
  echo: true
---

# Day 1: Introduction to "Reproducible Data Analysis"

## Instructor: Joel Nitta

:::: {.columns}

::: {.column width="50%"}
- Research topic 1: Ecology and evolution of **ferns**
  - Also teach [Biodiversity (Botany)](https://syllabus.gs.chiba-u.jp/2026/401001000000000/G15N945001/en_US)

- Research topic 2: Development of software for **data science**

- <https://www.joelnitta.com>
:::

::: {.column width="50%"}
![](https://github.com/joelnitta/joelnitta-site/blob/master/static/media/headers/research_img2.jpg?raw=true){fig-alt="Image of Joel Nitta in field" height="450"}
:::

::::

## Instructor: Joel Nitta

:::: {.columns}

::: {.column width="50%"}

- Born and raised in California

- Fourth generation Japanese-American

- First came to Japan as high school exchange student
:::

::: {.column width="50%"}
![](https://upload.wikimedia.org/wikipedia/commons/thumb/9/94/California_in_United_States.svg/960px-California_in_United_States.svg.png){fig-alt="Image of California map" width="100%" height="100%"}
:::

::::

## Ice-breaker

:::: {.columns}

::: {.column width="50%"}
- Answer the question: "Why are you interested in data analysis?"

- Introduce yourself and discuss with your neighbor
:::

::: {.column width="50%"}

![](https://farm4.staticflickr.com/3851/14762982307_9dbc2c46f5_o.jpg){fig-alt="Data analysis image" width="80%" height="80%"}

:::

::::

::: {.small}
https://www.odelama.com/data-analysis/
:::

## What is data analysis?

- Obtaining **insight** from **data**

- Important for **many careers** (academic and industry)

> Employment of data scientists is projected to grow **35%** from 2022 to 2032, **much faster than the average for all occupations**.

::: {.medsmall}
<https://www.bls.gov/ooh/math/data-scientists.htm>
:::

## Why programming?

Who has used Excel? Who has used a programming language?

What are the **advantages** and **disadvantages** of each for data analysis?

- Discuss with your neighbor

## Why programming? 

- Programming allows you to **record every step** of data analysis
  - This means you can **repeat your analysis!**

It takes some time to get used to, but eventually you will feel more comfortable with it because you can re-trace your steps and **have confidence** in your results.

## What about AI?

- Can't I just ask an AI to program everything for me?
  - Yes, but you need to know **what to ask it to do**

- This course will teach you the basics of programming **so that you can use AI effectively**
  - For example, you can't ask the AI to write a reproducible data analysis if you don't know how to run the code it produces

## Why reproducibility?

**When** might you want to repeat an analysis? **Why?**

- Discuss with your neighbor

## Why reproducibility? {.incremental}

- If new data comes in and you need to update an analysis

- If you want to double-check your own results

- If you want to repeat somebody else's analysis

- **If you switch between different projects and can't remember exactly what you were doing**

## Goals of this class

The goal of this class is to learn the fundamentals of reproducible data analysis by **doing it yourself**.

By the end of the course, you will be able to:

- load, clean, and visualize data using R
- track changes to code using Git and GitHub
- write a reproducible report using Quarto

## Expectations of this class

- I expect you to **participate in discussions**

- I expect you to **ask questions**

## Language of this class

- This class is **conducted in English**

- But, you can ask questions in Japanese and I will explain in Japanese if needed

## Recommended Reading

- R for Data Science (2nd ed.). <https://r4ds.hadley.nz/>
  - Available in Japanese from [Amazon](https://www.amazon.co.jp/R%E3%81%A7%E3%81%AF%E3%81%98%E3%82%81%E3%82%8B%E3%83%87%E3%83%BC%E3%82%BF%E3%82%B5%E3%82%A4%E3%82%A8%E3%83%B3%E3%82%B9-%E7%AC%AC2%E7%89%88-Hadley-Wickham/dp/4814400772/ref=sr_1_1?__mk_ja_JP=%E3%82%AB%E3%82%BF%E3%82%AB%E3%83%8A&crid=1RNKPXODXTLQ8&dib=eyJ2IjoiMSJ9.TRkyWspD1BdM3G_Etnt48M_btFEW7KgQZjdsnn9bPGhd_R8fuTqhdxZsyLCOIt7_2oSMjpg54cOei2e77U4gZpRG8DbPeUOazXZgGp0cN3XYGu6A6oMOGXpG8K_xvgg2dL7nByq90Fr00JrYgI4i2Q.nMU2x8-vkLOZIN5yZ7szm4iczNsvseG4JonghcPGwv0&dib_tag=se&keywords=R%E3%81%A7%E3%81%AF%E3%81%98%E3%82%81%E3%82%8B%E3%83%87%E3%83%BC%E3%82%BF%E3%82%B5%E3%82%A4%E3%82%A8%E3%83%B3%E3%82%B9+%E7%AC%AC2%E7%89%88&qid=1749698886&sprefix=%2Caps%2C410&sr=8-1) or [O'Reilly](https://www.oreilly.co.jp/books/9784814400775/)

- Happy Git with R. <https://happygitwithr.com/>

- Introduction to Reproducible Publications with RStudio <https://carpentries-incubator.github.io/reproducible-publications-quarto/>

## Homework assignments

- There will be a **homework assignment** on **GitHub** for each class, starting next week.

- You submit the assignment by making a **commit** in Git (more about this on Day 2)

## Final project

- You will need to analyze a dataset of your own choosing for your final project, due 2026-07-29 11:59 PM

- The last homework assignment is due 2026-07-15 11:59 PM, so you have 2 weeks to work on the final project

## Schedule

- Day 1 (2026-06-11): Introduction: Why code? Why reproducibility?

- Day 2 (2026-06-18): Git and GitHub

- Day 3 (2026-06-25): Basic usage of R and RStudio

- Day 4 (2026-07-02): Data loading and tidying with tidyverse

## Schedule (cont'd)

- Day 5 (Media Day): Joining data

- Day 6 (2026-07-09): Data visualization with ggplot2

- Day 7 (2026-07-16): Writing documents with Quarto

- Day 8 (2026-07-23): Quarto, part II

## Grades

- Homework (4 assignments) 50%
- Final report 50%

**No late submissions allowed** unless for a documented medical reason or family emergency

## Course website and slides

- All slides (and other info) available at<br><https://data-analysis-chiba.github.io/>

## Moodle

- **Assignments** (GitHub classroom repos) will be posted on Moodle

- **Check Moodle every week**

## Office hours

By appointment: contact me at <joelnitta@chiba.u-jp>

## Questions?

## AI {.incremental}

- Who has used AI (for example, ChatGPT) before?
  - <https://chat.openai.com/>

- You may use AI for your homework and final project

- But first you need to know **how to use it**

## AI {.incremental}

- AI **makes statistical predictions** about words based on training data (it does not "think")

- AI is designed to produce sentences that **sound** as natural as possible

- AI may **lie to you** or make up facts (called "hallucination"; this is especially common when it lacks adequate training data)

## AI policies (DOs) {.incremental}

- *Do* try by yourself first (without AI)

- *Do* ask it **detailed**, **specific** questions (prompts)

- *Do* **double-check** the results: does the AI's code produce the expected result?

- *Do* make sure you **understand** the code that the AI provides

## AI policies (DON'Ts) {.incremental}

- *Don't* copy-paste directly from AI for your report.
  - Typing the code yourself will help you remember it and understand what you are doing. **Copy-pasting text for a paper is plagiarism.**

- *Don't* submit an answer from AI without trying/checking it yourself first
  - The AI could very well be wrong!

## Setting up RStudio

- [Install R](https://cran.r-project.org/)

- [Install RStudio](https://posit.co/download/rstudio-desktop/)

## Setting up Git

We will follow instructions for [Day 2](../day2/#/setup) to set up Git
